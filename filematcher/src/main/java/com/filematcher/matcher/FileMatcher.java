...
public class FileMatcher {
	public static void main(String[] args) throws Exception {
		...
		List<PersonName> matchingStrings = fileMatcher.matchList(firstFileList,
				secndFileList);
		System.out.println("\n\nResult Size :" + matchingStrings.size()
				+ "\nResult element :" + matchingStrings);
	}

	public List<PersonName> matchList(List<PersonName> firstFileList,
			List<PersonName> secndFileList) {
		...
		for (PersonName personToBeMatched : firstFileList) {
			if (secndFileList.contains(personToBeMatched)) {
				...
				int indexOfMatchedName = secndFileList
						.indexOf(personToBeMatched);
				nameToNameMap.add("First Person :" + personToBeMatched
						+ "<-> Second Person :"
						+ secndFileList.get(indexOfMatchedName));
			}
		}
		return matchStringsList;
	}
